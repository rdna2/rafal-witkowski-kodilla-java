...
class BookDirectoryTestSuite {
    ...
    void testListBooksWithConditionMoreThan20() {
        ...
        List<Book> resultListOf40Books = generateListOfNBooks(21);                       // [4]
    ...
    void testListBooksInHandsOf() {
        ...
        //Given
        LibraryUser userBook0 = new LibraryUser("x", "y", "z");
        LibraryUser userBook1 = new LibraryUser("a", "b", "c");
        LibraryUser userBook5 = new LibraryUser("d", "e", "e");

        LibraryDatabase libraryDatabaseMock = mock(LibraryDatabase.class);
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);

        List<Book> resultList0Books = new ArrayList<>();
        List<Book> resultList1Books = generateListOfNBooks(1);
        List<Book> resultList5Books = generateListOfNBooks(5);

        when(libraryDatabaseMock.listBooksInHandsOf(userBook0)).thenReturn(resultList0Books);
        when(libraryDatabaseMock.listBooksInHandsOf(userBook1)).thenReturn(resultList1Books);
        when(libraryDatabaseMock.listBooksInHandsOf(userBook5)).thenReturn(resultList5Books);

        //When
        List<Book> userBook0List = bookLibrary.listBooksInHandsOf(userBook0);
        List<Book> userBook1List = bookLibrary.listBooksInHandsOf(userBook1);
        List<Book> userBook5List = bookLibrary.listBooksInHandsOf(userBook5);

        int value1 = userBook0List.size();
        int value2 = userBook1List.size();
        int value5 = userBook5List.size();

        // Then
        Assert.assertEquals(0, value1);
        Assert.assertEquals(1, value2);
        Assert.assertEquals(5, value5);
    }
}